# 📘 WEBREADME — Web 界面完整操作说明书

本文档是 SQL-boat-v2 Web 管理界面的详细操作指南，面向所有用户。
...
1. 已安装 Python 依赖：`pip install -e .`
2. 已配置 `config.yaml` 中的 VLM 服务地址
3. VLM 服务正在运行
...
点击某个视频卡片会**选选中**该视频，并显示 Pipeline 控制面板。
...
| 地址 | 说明 |
|------|------|
| `http://localhost:8000/docs` | Swagger UI（交互式文档，可直接测试） |
| `http://localhost:8000/redoc` | ReDoc（阅读式文档） |

也可以通过 `curl` 或任何 HTTP 客户端直接调用 API，示例：
...
# 上传图片识别
curl -X POST http://localhost:8000/api/ships/recognize \
  -F "file=@ship_photo.jpg"

# 搜索
curl "http://localhost:8000/api/ships/search?q=白色"
```
